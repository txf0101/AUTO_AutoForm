...
```powershell
claude mcp add-json autoform-mcp '{"type":"stdio","command":"conda","args":["run","-n","afagent","python","-m","autoform_mcp_agent.mcp_server"],"env":{"PYTHONPATH":"<repo-root>;<repo-root>\\AutoForm_MCP"}}'
```
...
3. 调用 `autoform_result_blockers`，查看 MCP_V1.8 的结果审阅边界。
4. 在没有确认许可证、工程、输出目录和可见桌面之前，不要给执行类工具传 `execute=true`。

## 当前能力

MCP_V1.8 暴露 112 个 `autoform_` 工具，并提供 `autoform://status` 资源。V1.8 保留 V1.1 的结果审阅范围，并补齐更精确的 GUI 续接路径：`autoform_result_set_view` 现在接收 `title_contains` 和 `target_pid`，host 可以把视角切换限定到当前 AutoForm 结果窗口，避免重新打开示例工程。`autoform_r12_project_view_demo` 新增 `view_sequence`；省略该参数时，工具只打开或规划工程窗口，不发送视角快捷键。常用工具包括：
