...
class AgentRuntimeBridge {
  ...
  async sendPrompt(prompt, options = {}) {
    // 网页 prompt 进入后端的主入口。
    // runtimeConfig 说明用哪个模型和 API 配置”，uiContext 说明“页面当前选择了什么工程操作，
    // conversationContext 则把上一轮工程、审批和脚本结果压缩后带回后端，保证多轮对话能接上。
    ...
    // fetch 只调用本机 HTTP bridge。前端不直接判断要不要打开 AutoForm，
    // 也不直接调用 MCP 工具；这些决定都交给 Python 后端和 AgentToolGateway。
...
function buildConversationContextForRequest() {
  // 这段代码决定“下一轮能不能记得上一轮”。
  // 只保留前端续接需要的小摘要：当前工程、待审批动作、材料候选、最近对话。
  // 完整日志和大块工具输出仍留在后端证据目录或命令输出区，避免请求越来越大。
...
function updateConversationContextFromReply(reply) {
  ...
  // 后端每轮会返回 runtime、centerPlan、toolRuns 和 agentMessages。
  // 前端从这些字段里抽取“后续还会用到的状态”，例如当前工程、待审批动作和材料候选。
  // 下一次 sendPrompt 会把这些摘要放回 conversationContext。
