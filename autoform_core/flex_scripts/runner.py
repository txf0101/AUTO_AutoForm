...
def _record_payload(
    ...
    *,
    ...
    return {
        ...
        "caller_agent": caller_agent,
        "executor": "autoform_core.flex_scripts.runner.subprocess",
...
def _subprocess_env() -> dict[str, str]:
    """Return an environment that lets stable library scripts import workspace packages."""

    env = os.environ.copy()
    current = env.get("PYTHONPATH")
    root_text = str(ROOT)
    env["PYTHONPATH"] = root_text if not current else f"{root_text}{os.pathsep}{current}"
    return env
