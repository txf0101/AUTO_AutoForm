...
def bridge_script_text(workspace: Path, python_executable: str | None = None) -> str:
    ...
    lines = [
        ...
        # root is added explicitly before calling the shared core bridge.
        f"set \"PYTHONPATH={workspace};%PYTHONPATH%\"",
        f'"{python_executable}" -m autoform_core.quicklink_bridge "%QUICKLINK_ARCHIVE%" --workspace "{workspace}"',
        "exit /b %ERRORLEVEL%",
        "",
    ]
    return "\n".join(lines)
...
def _quicklink_data_dir(workspace: Path) -> Path:
    ...
    return workspace.resolve() / "data/runtime/agent" / "quicklink"
