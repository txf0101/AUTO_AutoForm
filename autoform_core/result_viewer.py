...
def resolve_result_view(name: str) -> dict:
    ...
    } # 解析用户输入的视图名称，首先尝试与预定义视图的键或标签进行完全匹配，如果没有成功，则继续尝试与每个视图的同义词进行匹配。匹配成功时返回匹配类型和视图规范；如果所有尝试都失败，则返回未匹配状态和已知视图列表以供参考。
...
# 该函数首先将用户输入的视图参数解析为对应的标准快捷键，并在确认目标窗口可用后，向其发送快捷键以执行视图切换。若开启了截图校验，它还会自动截取操作前后的画面进行视觉差异比对，从而验证视图是否切换成功，并最终返回包含执行状态与证据的详细报告。
def set_result_view(
    view: str,
    *,
    execute: bool = False,
    ...
    selected_view = resolution["view"] # 从解析出的视图对象中获取对应的快捷键
...
def view_control_evidence_protocol(
    *,
    ...
    resolution = resolve_result_view(view) # 解析用户输入的 view 参数，看看能不能匹配到已知的视图类型和对应的控制方案。
    if not resolution["matched"]:
        return {
            "schema_version": "1.1",
            "status": "failed",
