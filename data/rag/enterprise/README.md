...
| `r25_process_rag_index_eval_queries.jsonl` | R25 候选索引检索评测查询，覆盖内部样本、合作企业输入、AutoForm 官网元数据、NIST 元数据和权限过滤。 |
| `r25_process_rag_index_eval_report.sample.json` | R25 候选索引检索评测报告，记录唯一键门禁、召回结果、排序解释、证据回链和正式索引阻断。 |
...
- 使用 R25 候选索引评测验证唯一键、过滤、排序、召回、证据回链和人工复核门禁。
...
- 让 R25 评测报告触发 embedding 计算、神经网络训练、正式索引写入、批量采集或工程状态写入。
