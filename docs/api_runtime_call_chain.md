...
网页工作台的“允许本机执行示例工程”只表达用户批准和示例工程选择。前端通过 `uiContext.localExecution` 发送该意图，后端运行时依据 prompt 与该上下文生成受控的 `autoform_project_run` 工具请求，并继续交给 `AgentToolGateway` 检查白名单、受控参数和批准状态。显式 `agentToolRequests` 仍保留给高级调用方，但不能借用网页示例工程批准绕过工具审批边界。
...
当 prompt 表达“新建工程”“创建工程”或启动 AutoForm 主界面的意图，但没有给出 `.afd` 路径或官方示例名时，后端会生成 `autoform_start_ui` 受控请求。该请求进入同一个 `AgentToolGateway`，未批准时返回 `blocked_requires_approval`，批准后启动 AutoForm Forming 主界面。当前白名单尚未包含自动填写 AutoForm 新建工程向导的工具，因此工程向导参数仍需用户在 GUI 内确认，或在后续新增更细粒度 MCP wrapper 后再自动化。
...
3. `frontend/app.js` 的 `DEFAULT_ENDPOINT` 指向 `http://127.0.0.1:4317/api/agent`，并在请求体中发送 `runtimeConfig`；当用户勾选本机示例工程执行时，页面还会发送 `uiContext.localExecution` 和批准状态，由后端运行时生成白名单工具请求。
