...
网页工作台的“允许本机执行和 AutoForm 控制”表达用户对本机受控动作的批准，并在展示工程场景中同时携带示例工程选择。前端通过 `uiContext.localExecution` 发送该意图，后端运行时依据 prompt 与该上下文生成受控的 `autoform_project_run` 或 `autoform_start_ui` 工具请求，并继续交给 `AgentToolGateway` 检查白名单、受控参数和批准状态。显式 `agentToolRequests` 仍保留给高级调用方，但不能借用网页批准绕过工具审批边界。

当 prompt 同时包含官方示例名和复制、打开窗口或求解意图时，后端会先生成 `autoform_resolve_project` 只读解析请求，再生成 `autoform_project_run` 受控请求。`LOCAL execution=disabled` 时，只读解析可以完成，`copy_project=true`、`open_gui=true` 和 `execute=true` 会由 `AgentToolGateway` 返回 `blocked_requires_approval`；用户批准后才会复制工程、打开 AutoForm 窗口或执行求解器。`open_gui=true` 与 `execute=false` 表示只复制安全运行副本并打开窗口，求解器保持 dry run。

当 prompt 表达“新建工程”“创建工程”或启动 AutoForm 主界面的意图，但没有给出 `.afd` 路径或官方示例名时，后端会生成 `autoform_start_ui` 受控请求。该请求进入同一个 `AgentToolGateway`，未批准时返回 `blocked_requires_approval`，回复文本会说明需要勾选前端本机执行开关；批准后启动 AutoForm Forming 主界面。当前白名单尚未包含自动填写 AutoForm 新建工程向导的工具，因此工程向导参数仍需用户在 GUI 内确认，或在后续新增更细粒度 MCP wrapper 后再自动化。
...
3. `frontend/app.js` 的 `DEFAULT_ENDPOINT` 指向 `http://127.0.0.1:4317/api/agent`，并在请求体中发送 `runtimeConfig`；当用户勾选本机执行和 AutoForm 控制时，页面还会发送 `uiContext.localExecution` 和批准状态，由后端运行时生成白名单工具请求。
...
6. `autoform_agent/mcp_server.py` 创建 `mcp = FastMCP("autoform-agent")` 并调用 `autoform_agent.mcp_tools.register_all_tools()`。`autoform_agent/mcp_tools/` 下的工具家族模块通过 `mcp.add_tool()` 注册 112 个 `autoform_` 工具，并在 status 模块中注册 `autoform://status` 只读资源。文件末尾在 `__main__` 情况下调用 `mcp.run()`，这是可选 MCP stdio 入口。
...
`frontend/` 是可视化层。它显示 prompt、状态总结、Agent 图谱、终端式输出、凭据边界和 API 使用情况。前端不决定 AutoForm 工具调用，只把 prompt 与 API 运行时配置发送到 HTTP bridge，并渲染后端返回结果。R5 后端响应会附带 `events`，页面按同一 `RunEvent` 外壳更新任务卡、路由、上下文视图、图谱、状态和 token 用量。Agent 图谱固定显示 9 个业务节点：中心Agent、需求与工艺规划Agent、几何与数据Agent、材料Agent、工艺设置Agent、求解执行Agent、后处理Agent、诊断与优化Agent、报告整理Agent；内部 role_id 会映射到这些节点，工作态为绿色，完成后回到待命灰白态。R19 起页面还识别 `tool_requested`、`tool_completed`、`tool_blocked`、`tool_failed` 和 `approval_required`，用于展示网关工具状态。
