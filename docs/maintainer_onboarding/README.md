...
1. 用户界面层：`apps/workbench/` 和 `start_autoform_agent.ps1`。
...
5. AutoForm 业务能力层：`autoform_core/project_workflow.py`、`solver.py`、`results.py`、`result_viewer.py` 等。
6. MCP wrapper 层：`autoform_core/tool_registry/` 和 `AutoForm_MCP/autoform_mcp_agent/mcp_server.py`。
7. 测试与复盘层：`tests/`、`apps/workbench/tests/`、`handoff/`。
