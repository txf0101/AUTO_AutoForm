...
| 能力或事实 | 依据 |
| --- | --- |
| MCP 已能运行工程并打开 GUI 观察窗口 | `autoform_core/tool_registry/project.py` 中的 `autoform_project_run(open_gui=True)` |
| CLI 已有窗口枚举、恢复、聚焦、截图、坐标点击能力 | `autoform_core/gui_automation.py` 与 `autoform_agent/cli.py` 中的 `gui-window-snapshot`、`gui-restore-window`、`gui-focus`、`gui-screenshot`、`gui-click` |
...
| 当前 MCP 已接入 GUI 和结果审阅工具层 | `autoform_core/tool_registry/__init__.py` 当前注册 status、project、jobs、materials、quicklink、environment、queue、solver、commands、reporting、release、reference 和 gui 共 13 个工具层，工具总数为 112 |
...
| P1 高层审阅计划已形成结构化入口 | `autoform_core/result_viewer.py` 的 `build_result_review_plan()`、CLI `result-plan`、MCP `autoform_result_plan_review` 和 Agent runtime `autoform_result_review_plan_tool` |
| GUI 就绪诊断已形成结构化入口 | `autoform_core/result_viewer.py` 的 `assess_result_review_readiness()`、CLI `result-readiness`、MCP `autoform_result_readiness` 和 Agent runtime `autoform_result_readiness_tool` |
| 桌面观察就绪探针已形成结构化入口 | `autoform_core/gui_automation.py` 的 `computer_use_probe()`、CLI `computer-use-probe`、MCP `autoform_computer_use_probe` 和 Agent runtime `autoform_computer_use_probe_tool`；当前会话截图尝试返回 `screen grab failed` |
| GUI 控件证据登记已形成结构化入口 | `autoform_core/result_viewer.py` 的 `result_gui_evidence()`、CLI `result-gui-evidence`、MCP `autoform_result_gui_evidence` 和 Agent runtime `autoform_result_gui_evidence_tool` |
...
| 视角切换取证已有准备入口，主要快捷键已验证 | `autoform_core/result_viewer.py` 的 `view_control_evidence_protocol()`、CLI `result-view-evidence`、MCP `autoform_result_view_evidence` 和 Agent runtime `autoform_result_view_evidence_tool` 已覆盖等轴测、俯视、正视、侧视、适合窗口和复位的 before、after、compare 流程；2026-06-01 对 AutoComp_R13 窗口实测确认 `E`、`Z`、`X` 和 `Shift+Y` 自动切换有效 |
...
```text
autoform_core/tool_registry/gui.py
```
...
```text
autoform_core/result_viewer.py
autoform_core/tool_registry/gui.py
