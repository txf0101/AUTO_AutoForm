...
1. 将 `frontend/index.html` 改为 P0 工作台结构，覆盖用户输入、状态总结、Agent 图谱、命令输出、凭据边界和 token 用量。
2. 将 `frontend/app.js` 改为 `RunEvent` fixture 回放驱动，同时保留 `AgentRuntimeBridge.sendPrompt()` 的 HTTP bridge 路径。
3. 将 `frontend/styles.css` 改为三列工程工作台布局，并补充 1160、980 和 620 宽度断点。
4. 将启动器静态服务根目录改为仓库根目录，访问路径改为 `/frontend/index.html?bridge=http`，保证页面能读取根目录 `fixtures/`。
5. 更新 `frontend/README.md`、`README.md`、`docs/beginner_onboarding_zh.md` 和前端 smoke 测试。

## 检查结果

- `python -m pytest frontend\tests tests\test_p0_contracts.py tests\test_launcher_scripts.py -q --basetemp=tmp\pytest_frontend_r3_fix` 通过。
- Bundled Node 执行 `frontend/tests/smoke-test.mjs` 通过，`frontend/app.js` 语法检查通过。
