...
| R3 | 黑白静态 UI 与本地回放 | 通过 | `apps/workbench/tests` pytest 通过；Node smoke test 返回 `frontend smoke test passed` |
...
| R10 | 柔性脚本 L0 至 L2 | 未完成 | `script_library/flex/registry.yaml` 不存在；`SkillCard`、`ScriptRunRecord`、`FailureSummary` 只在命名或权限文档中出现 |
...
```powershell
python -m pytest apps\workbench\tests -q --basetemp=tmp\pytest_verify_r3_apps/workbench
```
...
4. R10 只做 L0 至 L2 低风险脚本，先建 `script_library/flex/registry.yaml`、`ConsoleLine` 和失败摘要测试。
