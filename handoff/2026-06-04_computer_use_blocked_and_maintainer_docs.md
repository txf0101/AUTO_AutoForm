...
- `apps/workbench/app.js`：补充九个业务 Agent 图谱、内部 role_id 映射、运行态复位和边过滤注释。
- `apps/workbench/styles.css`：补充三列九宫格布局和绿色运行态规则注释。
...
结果：`frontend smoke test passed`，`apps/workbench/app.js` 语法检查通过。
