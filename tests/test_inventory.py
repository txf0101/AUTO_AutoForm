"""这个测试文件检查安装清点、示例工程和 AFD 可读索引。读测试时可以把每个断言看成一条项目承诺：输入什么、应该返回什么、哪些危险动作默认不能发生。

This test file checks installation inventory, example projects, and readable AFD indexes. Read each assertion as one project promise: what input is accepted, what output must come back, and which risky actions must stay disabled by default.
"""
