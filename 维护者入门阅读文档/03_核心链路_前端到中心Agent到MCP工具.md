...
frontend/app.js
  ...
  -> autoform_agent/mcp_tools/*
  -> autoform_agent/project_workflow.py 等业务模块
```
...
`autoform_agent/mcp_tools/` 是 MCP wrapper 层。它有两种使用方式：

1. 外部 MCP host 启动 `python -m autoform_agent.mcp_server` 后直接调用这些工具。
...
后端返回的 `events` 会被 `frontend/app.js` 的 `applyRunEvent()` 消费。关键规则：
